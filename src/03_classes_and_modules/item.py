...
class Sculpture(Item):
    ...
    def __str__(self):
        ...
        return "{}{}".format(super(Sculpture, self).__str__(), materialString)
...
if __name__ == "__main__":
    ...
    items.append(Sculpture("Auguste Rodin", "Eternal Springtime", 1917, "plaster"))
    items.append(Sculpture("Auguste Rodin", "Naked Balzac", 1917, "plaster"))
    items.append(Sculpture("Auguste Rodin", "The Secret", 1925, "bronze"))
    uniquematerials = set()
    for item in items:
        print(item)
        if hasattr(item, "material"):
            uniquematerials.add(item.material())
    print("Sculptures use {} unique materials".format(len(uniquematerials)))
